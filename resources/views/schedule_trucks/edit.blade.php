    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
                <form action="{{ route('schedule_trucks.update', $schedule_truck->id) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select id="driver_id" name="driver_id" required
                                ...
                                @foreach ($drivers as $driver)
                                    <option value="{{ $driver->id }}" {{ $schedule_truck->driver_id == $driver->id ? 'selected' : '' }}>
                                        {{ $driver->Name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="truck_id" name="truck_id" required
                                ...
                                @foreach ($trucks as $truck)
                                    <option value="{{ $truck->id }}" {{ $schedule_truck->truck_id == $truck->id ? 'selected' : '' }}>
                                        {{ $truck->Plate_number }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="text" id="cargo" name="cargo" value="{{ $schedule_truck->cargo }}" required
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:text-white">
                        </div>

                        <!-- Added Destination Field here -->
                        <div>
                            <label for="destination" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Destination</label>
                            <input type="text" id="destination" name="destination" value="{{ $schedule_truck->destination }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:text-white">
                        </div>
